...
//                      println(max,min)
    ...
    //    print()
    //    //  }
//    var arr = Array(9, 3, 2, 4, 5, 6, 8, 10, 12)
//    var n = arr.length
//    for (i <- 0 until n) {
//      for (j <- 0 until n - i - 1) {
//        if (arr(j) > arr(j + 1)) {
...
//      }
      ...
      //        if (arr(i)==targetdata)
      //          {
      //            println("element is present at that index:",i)
      //          }
      //      }

//val arr=Array(1,2,3,4,5,6,7,10,12,14,17,19,20)
...
//    }
    ...
    var str="this is the first sentence,that i have written in scala"
    var count=0
    for(ch<-str)
      {
        if(ch.toLower=='a' || ch.toLower=='e' || ch.toLower=='i' || ch.toLower=='o' || ch.toLower=='u')
          {
            count=count+1
          }

      }
    print("the no of vowls are:"+count)
